...
for(n in train_vec) {
  ...
  for(o in obs) {
    ...
    for(i in 1:folds) {
      ...
      randsamp <- sample(1:nrow(X), nrow(X), replace = FALSE)
      Xtot <- X[randsamp,]
      Ytot <- Y[randsamp]
...
saveRDS(final_list, file = "final_list.rds")

final_list <- readRDS("mix_dat.rds")
...
write.csv(result_frame, file = "sim_result.csv", row.names = FALSE)
